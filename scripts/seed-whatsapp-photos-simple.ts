...
async function seedWhatsAppPhotos() {
  ...
  try {
    // Note: We need a dummy user_id and approval_status for photos table
    // First, let's check if we have a user
    ...
    // We'll use dummy media ID (0) since we're not using Payload's media system
    const dummyMediaId = 0;
    ...
    const garlabanPhotoIds: number[] = [];
    for (const photo of garlabanPhotos) {
      const result = await pool.query(
        `INSERT INTO photos (title, description, image_url_id, category, uploaded_by_id, approval_status)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        ["Randonnée Garlaban", photo.desc, dummyMediaId, "event", userId, "approved"]
      );
...
Un grand merci à toutes les familles et participants qui étaient présents pour cette randonnée. Ce fut un moment convivial, simple et chaleureux, exactement dans l'esprit que l'on souhaite faire vivre au club.`,
    ...
    const trainingPhotoIds: number[] = [];
    for (const photo of trainingPhotos) {
      const result = await pool.query(
        `INSERT INTO photos (title, description, image_url_id, category, uploaded_by_id, approval_status)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        ["Entraînement extérieur", photo.desc, dummyMediaId, "training", userId, "approved"]
      );
    ...
    const gymPhotoIds: number[] = [];
    for (const photo of gymPhotos) {
      const result = await pool.query(
        `INSERT INTO photos (title, description, image_url_id, category, uploaded_by_id, approval_status)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        ["Travaux gymnase", photo.desc, dummyMediaId, "club", userId, "approved"]
      );
...
À très vite sur les terrains! 💜💛`,
    ...
    const bf1PhotoIds: number[] = [];
    for (const photo of bf1Photos) {
      const result = await pool.query(
        `INSERT INTO photos (title, description, image_url_id, category, uploaded_by_id, approval_status)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        ["Diplôme BF1", photo.desc, dummyMediaId, "club", userId, "approved"]
      );
